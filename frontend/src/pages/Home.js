...
const HOME_PRODUCT_IMAGES = {
  ...
  "prod-5": "/images/ai-bottle-5.jpeg",
};
...
export default function Home() {
  ...
  return (
    <div data-testid="home-page">
      {/* HERO */}
      <section className="relative bg-cream overflow-hidden home-hero-section">
        ...
        <div className="container-drj home-hero-inner relative grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10">
            ...
            <p className="text-[var(--drj-ink-muted)] mt-6 max-w-xl text-lg font-light leading-relaxed">
              {t.hero.desc}
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-6 text-xs text-[var(--drj-ink-muted)] uppercase tracking-[0.2em]">
              <span className="flex items-center gap-2 text-forest"><Star size={14} className="fill-gold text-gold" /> {t.hero.reviews}</span>
            </div>

            <Link to="/about" className="mt-6 inline-flex items-center gap-3 px-4 py-3 bg-white border border-[var(--drj-line)] hover:border-gold transition group max-w-md" data-testid="hero-founder-badge">
              <img src={BRAND.founder.photo} alt="" className="w-12 h-12 rounded-full object-cover border-2 border-gold" />
              <div className="text-left">
                <div className="text-overline text-gold">{t.founder.note}</div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: 0.15 }} className="relative z-10">
            <div className="relative aspect-square max-w-lg mx-auto w-full flex items-center justify-center">
              ...
              <img
                ...
                className="relative z-10 max-h-[310%] w-auto object-contain mx-auto drop-shadow-xl"
              />
              <div className="absolute -bottom-4 -right-4 bg-white border border-[var(--drj-gold)] p-5 shadow-xl">
                <div className="text-overline text-gold">{t.hero.from}</div>
                <div className="font-serif text-3xl text-forest leading-none mt-1">₹999</div>
                <div className="text-xs text-[var(--drj-ink-muted)] mt-1">{t.hero.pack_size}</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="section bg-cream">
        <div className="container-drj">
          <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
            <div>
              <div className="drj-divider text-overline mb-4">{t.home.apothecary_eyebrow}</div>
              <h2 className="font-serif text-4xl lg:text-5xl text-forest tracking-tight">{t.home.apothecary_title}</h2>
            </div>
            <Link to="/shop" className="btn-outline" data-testid="shop-all-cta">{t.home.view_all} <ArrowRight size={16}/></Link>
          </div>
      ...
      <section className="section bg-white border-t border-[var(--drj-line)]">
        <div className="container-drj text-center mb-16">
          <div className="drj-divider text-overline mb-4">{t.home.instagram_eyebrow}</div>
      ...
      <section className="section bg-cream">
        <div className="container-drj grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-4">
            <div className="drj-divider text-overline mb-4">{t.home.faq_eyebrow}</div>
            <h2 className="font-serif text-4xl lg:text-5xl text-forest tracking-tight">{t.home.faq_title}</h2>
            <p className="text-[var(--drj-ink-muted)] mt-5 font-light">{t.home.faq_desc}</p>
            <a href={whatsappLink()} target="_blank" rel="noreferrer" className="btn-outline-gold mt-6" data-testid="faq-whatsapp-cta">
              <i className="fa-brands fa-whatsapp"></i> {t.home.faq_whatsapp}
            </a>
          </div>
      ...
      <section className="bg-forest text-white relative overflow-hidden">
        ...
        <div className="container-drj py-20 lg:py-24 grid lg:grid-cols-2 gap-10 items-center relative">
          <div>
            <div className="text-overline text-[var(--drj-gold-bright)] mb-3">{t.home.cta_eyebrow}</div>
            <h2 className="font-serif text-4xl lg:text-5xl tracking-tight leading-tight">{t.home.cta_title}</h2>
          </div>
