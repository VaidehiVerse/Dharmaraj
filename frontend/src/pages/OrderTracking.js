...
export default function OrderTracking() {
  ...
  const currentIdx = order ? ALL_STATUSES.findIndex((s) => s.key === order.status) : -1;

  return (
    <div data-testid="track-page" className="bg-[var(--drj-bg)] min-h-screen">
      <section className="bg-obsidian text-white relative grain">
        <div className="container-drj py-20 lg:py-28 relative">
          ...
          <h1 className="font-serif text-5xl lg:text-6xl mt-3 tracking-tight">Track Your Order</h1>
          <p className="text-white/60 mt-4 max-w-xl font-light">Enter your order number and the mobile used at checkout to see where your ritual is.</p>
        </div>
      </section>
      ...
      <div className="container-drj py-12 lg:py-20 grid lg:grid-cols-3 gap-10">
        <form onSubmit={track} className="bg-white border border-[var(--drj-line)] p-8 lg:col-span-1 self-start space-y-6" data-testid="track-form">
          ...
          <button disabled={loading} className="btn-primary w-full justify-center" data-testid="track-submit">{loading ? "Looking up..." : "Track Order"}</button>
        </form>
          ...
          {order ? (
            <div className="bg-white border border-[var(--drj-line)] p-8" data-testid="track-results">
              ...
              <div className="grid sm:grid-cols-2 gap-8 mt-10 pt-8 border-t border-[var(--drj-line)] text-sm">
                <div>
                  <div className="text-overline text-gold mb-2">Shipping To</div>
                ...
                <div>
                  <div className="text-overline text-gold mb-2">Items</div>
            ...
            <div className="bg-white border border-dashed border-[var(--drj-line)] p-12 text-center text-[var(--drj-ink-muted)] font-light">
              ...
              Enter your order details to view tracking.
            </div>
          )}
        </div>
      </div>
